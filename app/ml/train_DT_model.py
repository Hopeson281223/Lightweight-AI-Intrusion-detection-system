...
def save_training_metrics(model_name, dataset_name, metrics, model_size_kb, feature_count, training_time):
    ...
    try:
        ...
        save_metrics(f"model_{model_name}_false_positive_rate", f"{metrics['false_positive_rate']:.4f}")
        save_metrics(f"model_{model_name}_false_negative_rate", f"{metrics['false_negative_rate']:.4f}")
        save_metrics(f"model_{model_name}_true_positive_rate", f"{metrics['true_positive_rate']:.4f}")
...
def main():
    ...
    np.save(PREPROCESSED_DIR / "CIC_ALL_X_test.npy", X_test)
    np.save(PREPROCESSED_DIR / "CIC_ALL_y_test.npy", y_test)
    print(f"[INFO] Test set saved for future evaluation")
    ...
    print(f"False Positive Rate: {metrics['false_positive_rate']:.4f}")  # Optional
    print(f"False Negative Rate: {metrics['false_negative_rate']:.4f}")  # Optional
